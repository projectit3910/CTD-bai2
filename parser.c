...
void compileCallSt(void) {
  ...
  if (isPredefinedProcedure(proc)) {
    ...
    genINT(4);
    compileArguments(proc->procAttrs->paramList);
    genDCT(4 + proc->procAttrs->paramSize);
...
Type* compileFactor(void) {
  ...
  case TK_FLOAT: // extra
    eat(TK_FLOAT);
    type = floatType;
    genLC(float2WORD(currentToken->fvalue));
    break;
  case TK_STRING: // extra
    eat(TK_STRING);
    type = stringType;
    genLCS(currentToken->string);
    break;
  case TK_CHAR:
    eat(TK_CHAR);
    type = charType;
  ...
  case TK_IDENT:
    ...
    case OBJ_CONSTANT:
      ...
      case TP_FLOAT:
        type = floatType;
        genLC(float2WORD(obj->constAttrs->value->floatValue));
        break;
      case TP_STRING:
        type = stringType;
        genLCS(obj->constAttrs->value->stringValue);
        break;
      default:
        break;
      }
    ...
    case OBJ_FUNCTION:
      if (isPredefinedFunction(obj)) {
        ...
        genINT(4);
        compileArguments(obj->funcAttrs->paramList);
        genDCT(4 + obj->funcAttrs->paramSize);
        genFunctionCall(obj);
      }
